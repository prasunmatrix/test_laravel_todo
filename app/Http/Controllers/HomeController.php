...
class HomeController extends Controller
{
  ...
  public function showPage(Request $request)
  {
    ...
    if (isset($news)) {
      ...
      $next_record = Page::where('page_number', '>', $pageData->page_number)->where('news_id', $news->id)->orderBy('page_number', 'ASC')->first();
      ...
      $previous_record = Page::where('page_number', '<', $pageData->page_number)->where('news_id', $news->id)->orderBy('page_number', 'DESC')->first();
    ...
    $prev = "'prev'";
    $next = "'next'";
    $prev_arrow=asset('assets/frontend/images/arrow_left.svg');
    $next_arrow=asset('assets/frontend/images/arrow_right.svg');
    $arr = array('status' => 1, 'templateData' => '<a href="javascript:void(0);" class="prev-arrow"  onclick="pageChange(' . $previous_page_id . ',' . $prev . ')"><img src="'.$prev_arrow.'" alt="Prev Arrow image"></a>' . $pageData->template . '<a href="javascript:void(0);" class="next-arrow"  onclick="pageChange(' . $next_page_id . ',' . $next . ')"><img src="'.$next_arrow.'" alt="Next Arrow image"></a>', 'id' => $pageData->id, 'latestNewsPageALLData' => $latestNewsPageALLData);
    //dd($arr);
  ...
  public function viewParticularDateData(Request $request)
  {
    ...
    if ($newsCount > 0) {
      ...
      if (isset($datePickuptNews)) {
        ...
        $next_record = Page::where('page_number', '>', $datePickuptNewsPageData->page_number)->where('news_id', $datePickuptNews->id)->orderBy('page_number', 'ASC')->first();
        if (isset($next_record)) {
          $next_page_id = $next_record->id;
        } else {
          $next_page_id = 0;
        ...
        $previous_record = Page::where('page_number', '<', $datePickuptNewsPageData->page_number)->where('news_id', $datePickuptNews->id)->orderBy('page_number', 'DESC')->first();
        //dd($previous_record->id);
        if (isset($previous_record)) {
          $previous_page_id = $previous_record->id;
        } else {
        ...
        if (isset($datePickuptNewsPageData)) {
          ...
          $prev_arrow=asset('assets/frontend/images/arrow_left.svg');
          $next_arrow=asset('assets/frontend/images/arrow_right.svg');
          $arr = array('status' => 1, 'datePickuptNewsPageData' => '<a href="javascript:void(0);" class="prev-arrow" onclick="pageChangePreviousDate(' . $previous_page_id . ',' . $prev . ')"><img src="'.$prev_arrow.'" alt="Prev Arrow image"></a>' . $datePickuptNewsPageData->template . '<a href="javascript:void(0);" class="next-arrow" onclick="pageChangePreviousDate(' . $next_page_id . ',' . $next . ')"><img src="'.$next_arrow.'" alt="Next Arrow image"></a>', 'datePickuptNewsPageALLData' => $datePickuptNewsPageALLData, 'changeDate' => $changeDate);
          echo json_encode($arr);
        } else {
  ...
  public function showPagePrevious(Request $request)
  {
    ...
    $prev_arrow=asset('assets/frontend/images/arrow_left.svg');
    $next_arrow=asset('assets/frontend/images/arrow_right.svg');
    $arr = array('status' => 1, 'templateData' => '<a href="javascript:void(0);" class="prev-arrow" onclick="pageChangePreviousDate(' . $previous_page_id . ',' . $prev . ')"><img src="'.$prev_arrow.'" alt="Prev Arrow image"></a>' . $pageData->template . '<a href="javascript:void(0);" class="next-arrow" onclick="pageChangePreviousDate(' . $next_page_id . ',' . $next . ')"><img src="'.$next_arrow.'" alt="Next Arrow image"></a>', 'id' => $pageData->id, 'latestNewsPageALLData' => $latestNewsPageALLData, 'dateData' => $dateData);
    //dd($arr);
    //echo json_encode($arr);
    return json_encode($arr);
  }
  ...
  public function pdfPreview(Request $request)
  {
    ...
    $returnHTML= view('frontend.pdfpreview')->with('pageData', $pageData)->render();
  ...
  public function pdfView(Request $request)
  {
    ...
    $pageData = Page::where('id', $id)->first();
    $data = [
      'templateData' => $pageData->template
    ];
    $pdf = PDF::loadView('frontend.pdfview', $data);
    //dd($pdf);
