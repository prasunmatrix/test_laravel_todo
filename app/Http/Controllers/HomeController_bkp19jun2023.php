...
class HomeController extends Controller
{
  ...
  public function showPage(Request $request)
  {
    ...
    $arr = array('status' => 1, 'templateData' => '<a href="javascript:void(0);" onclick="pageChange(' . $previous_page_id . ')">Prev</a>' . $pageData->template . '<a href="javascript:void(0);" onclick="pageChange(' . $next_page_id . ')">Next</a>', 'id' => $pageData->id, 'latestNewsPageALLData' => $latestNewsPageALLData);
    //dd($arr);
    ...
    if ($newsCount > 0) {
      ...
      if (isset($datePickuptNews)) {
        ...
        if (isset($datePickuptNewsPageData)) {
          ...
          $arr = array('status' => 1, 'datePickuptNewsPageData' =>'<a href="javascript:void(0);" onclick="pageChangePreviousDate(' . $previous_page_id . ')">Prev</a>' . $datePickuptNewsPageData->template . '<a href="javascript:void(0);" onclick="pageChangePreviousDate(' . $next_page_id . ')">Next</a>', 'datePickuptNewsPageALLData' => $datePickuptNewsPageALLData, 'changeDate' => $changeDate);
          echo json_encode($arr);
        } else {
  ...
  public function showPagePrevious(Request $request)
  {
    ...
    //dd($latestNewsPageALLData);
    //$arr = array('status' => 1, 'templateData' => $pageData->template, 'id' => $pageData->id, 'latestNewsPageALLData' => $latestNewsPageALLData);
    $arr = array('status' => 1, 'templateData' => '<a href="javascript:void(0);" onclick="pageChangePreviousDate(' . $previous_page_id . ')">Prev</a>' . $pageData->template . '<a href="javascript:void(0);" onclick="pageChangePreviousDate(' . $next_page_id . ')">Next</a>', 'id' => $pageData->id, 'latestNewsPageALLData' => $latestNewsPageALLData,'dateData'=>$dateData);
    //dd($arr);
    //echo json_encode($arr);
    return json_encode($arr);
  }
